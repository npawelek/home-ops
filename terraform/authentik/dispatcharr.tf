resource "authentik_provider_proxy" "dispatcharr" {
  name                         = "dispatcharr-proxy"
  access_token_validity        = var.access_token_validity
  authorization_flow           = data.authentik_flow.default_authorization_flow.id
  invalidation_flow            = data.authentik_flow.default_invalidation_flow.id
  external_host                = "https://dispatcharr.${var.domain}"
...
resource "authentik_outpost_provider_attachment" "dispatcharr" {
  outpost           = data.authentik_outpost.embedded.id
  protocol_provider = authentik_provider_proxy.dispatcharr.id
}

resource "authentik_policy_binding" "dispatcharr_admins_access" {
